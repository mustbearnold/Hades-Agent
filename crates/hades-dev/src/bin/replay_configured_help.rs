//! `hades-dev replay-configured-help` — Rust port of
//! `scripts/replay_configured_help.py` (HAD-148).
//!
//! Replays the configured /help panel against the OBS-0098 contract: a
//! fresh configured Hades process at 120x40 (absent loopback endpoint, no
//! credentials), the /help command opening the stable bordered panel
//! (3 consecutive identical samples), main-surface landmarks still
//! rendered, no setup-required state, and a clean Ctrl+C exit with
...
use hades_dev::replay::{
    ExitStatus, RetainedSlave, marker_present, spawn_with_env, terminal_flags, wait_for,
    wait_for_exit,
};
...
/// Reconstruct the help-panel state exactly like the Python `help_state`:
/// the command row with its exact top/bottom borders and the composer
/// draft, or None while the panel has not converged.
fn help_state(raw: &[u8]) -> Option<Value> {
    ...
    // The command row itself is only asserted to exist (contains
    // "Show available commands"); the recorded command text is the
    // contract constant.
...
fn wait_for_stable_help(
    ...
    while Instant::now() < deadline {
        ...
        if hades_dev::replay::try_wait(&child.child)?.is_some() {
            return Err(
                "configured-help-panel: Hades exited before the help panel stabilized".to_owned()
            );
        }
        std::thread::sleep(Duration::from_millis(50));
    }
    Err(format!(
        "configured-help-panel: timed out after {:.1}s waiting for the exact help panel",
        timeout.as_secs_f64()
    ))
}

fn run_case(binary: &Path, timeout: Duration) -> Result<Value, String> {
    let root = std::env::temp_dir().join(format!("hades-configured-help-{}", std::process::id()));
    ...
    let result = (|| -> Result<Value, String> {
        ...
        hades_dev::replay::send(&child.child.master, b"/help\r")
            .map_err(|error| error.to_string())?;
        let (panel, stable_samples) = wait_for_stable_help(&child, &mut output, timeout)?;

        let lines = screen_lines(&output);
        for marker in MAIN_SURFACE_MARKERS {
            if !lines.iter().any(|line| marker_present(line, marker)) {
                return Err(format!("configured-help-panel: missing stable marker: {marker}"));
            }
        }
        if lines.iter().any(|line| marker_present(line, "Setup Required")) {
            return Err(
                "configured-help-panel: configured /help entered setup-required state".to_owned()
            );
        }
        ...
        Ok(json!({
            ...
            "main_surface_markers": MAIN_SURFACE_MARKERS,
            "help_panel": panel,
