#!/usr/bin/env python3
"""Replay the bounded Hades setup-wizard contract in an isolated tmux PTY."""
...
def load_contract(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(contract, dict) or contract.get("schema_version") != 1:
        raise ComposerReplayFailure("contract", "load", "unsupported setup-wizard contract schema")
    cases = contract.get("cases")
    if not isinstance(cases, list) or not cases:
        raise ComposerReplayFailure("contract", "load", "setup-wizard contract has no cases")
    for case in cases:
...
def send_input(session: str, input_value: dict[str, str]) -> None:
    if input_value["kind"] == "text":
        ...
        try:
            ...
            raise ComposerReplayFailure("input", input_value["value"], "unsupported setup-wizard key") from error
...
def run_case(binary: Path, case: dict[str, Any], timeout: float, ordinal: int) -> dict[str, Any]:
    case_id = case["id"]
    session = f"had042-setup-{ordinal}-{int(time.time() * 1000)}"
    history_home = Path(tempfile.mkdtemp(prefix="had042-setup-history-"))
