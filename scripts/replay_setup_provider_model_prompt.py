#!/usr/bin/env python3
"""Replay the bounded Hades provider-selection model-prompt contract in tmux."""
...
def load_contract(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(contract, dict) or contract.get("schema_version") != 1:
        raise ComposerReplayFailure("contract", "load", "unsupported model-prompt contract schema")
    cases = contract.get("cases")
    if not isinstance(cases, list) or not cases:
        raise ComposerReplayFailure("contract", "load", "model-prompt contract has no cases")
    for case in cases:
...
def send_input(session: str, input_value: dict[str, str]) -> None:
    if input_value["kind"] == "text":
        ...
        try:
            ...
            raise ComposerReplayFailure("input", input_value["value"], "unsupported model-prompt key") from error
...
def main() -> int:
    ...
    report: dict[str, Any] = {
        "schema_version": 1,
        "command": "replay-setup-provider-model-prompt",
        "passed": False,
        "binary": "<hades-binary>",
