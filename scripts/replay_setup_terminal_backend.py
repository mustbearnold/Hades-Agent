#!/usr/bin/env python3
"""Replay the bounded Hades terminal-backend picker contract in tmux."""
...
KEY_PAYLOADS = {"Enter": "C-m", "Down": "Down", "Ctrl+C": "C-c"}
...
def load_contract(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(contract, dict) or contract.get("schema_version") != 1:
        raise ComposerReplayFailure("contract", "load", "unsupported terminal-backend contract schema")
    cases = contract.get("cases")
    if not isinstance(cases, list) or not cases:
        raise ComposerReplayFailure("contract", "load", "terminal-backend contract has no cases")
    for case in cases:
    ...
    if input_value["kind"] == "text":
        ...
        try:
            ...
            raise ComposerReplayFailure(
                "input", input_value["value"], "unsupported terminal-backend key"
            ) from error
...
def run_case(binary: Path, case: dict[str, Any], timeout: float, ordinal: int) -> dict[str, Any]:
    case_id = case["id"]
    session = f"had051-terminal-backend-{ordinal}-{int(time.time() * 1000)}"
    history_home = Path(tempfile.mkdtemp(prefix="had051-terminal-backend-history-"))
...
def main() -> int:
    ...
    report: dict[str, Any] = {
        "schema_version": 1,
        "command": "replay-setup-terminal-backend",
        "passed": False,
        "binary": "<hades-binary>",
