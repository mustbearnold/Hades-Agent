#!/usr/bin/env python3
"""Replay the bounded Hades model-picker contract in an isolated tmux PTY."""
...
def load_contract(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(contract, dict) or contract.get("schema_version") != 1:
        raise ComposerReplayFailure("contract", "load", "unsupported model-picker contract schema")
    cases = contract.get("cases")
    if not isinstance(cases, list) or not cases:
        raise ComposerReplayFailure("contract", "load", "model-picker contract has no cases")
    for case in cases:
...
def send_input(session: str, input_value: dict[str, str]) -> None:
    if input_value["kind"] == "text":
        ...
        payloads = {"Enter": "C-m", "Escape": "Escape", "Ctrl+C": "C-c"}
        try:
            payload = payloads[input_value["value"]]
        except KeyError as error:
            raise ComposerReplayFailure("input", input_value["value"], "unsupported model-picker key") from error
...
def run_case(binary: Path, case: dict[str, Any], timeout: float, ordinal: int) -> dict[str, Any]:
    case_id = case["id"]
    session = f"had040-model-{ordinal}-{int(time.time() * 1000)}"
    history_home = Path(tempfile.mkdtemp(prefix="had040-model-history-"))
